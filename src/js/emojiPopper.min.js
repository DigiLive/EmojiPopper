...
9),h=['<div class="accordion" id="emojiPopper'+e+'" data-type="emojiPopper">'],f="true",d="show";a.each(b,function(a,c){c.groupName!=g&&(g=c.groupName,h.push('<div class="card"><div class="card-header p-0" id="group'+a+'">'),h.push('<button type="button" class="btn btn-link btn-block" data-toggle="collapse" data-target="#collapse'+a+'" aria-expanded="'+f+'" aria-controls="collapse'+a+'">'),h.push(c.groupName),h.push("</button></div>"),h.push('<div id="collapse'+a+'" class="collapse '+d+'" aria-labelledby="group'+
a+'" data-parent="#emojiPopper'+e+'">'),h.push('<div class="card-body">'));h.push('<span title="'+c.description+'">'+c.formatted+"</span>");a+1 in b&&b[a+1].groupName==g||(h.push("</div></div></div>"),f="false",d="")});h.push("</div>");return h.join("")}function k(b){n=c(b);a(document).on("click",'[data-type="emojiPopper"] span',function(b){var e;b=a(this).closest('[data-type="emojiPopper"]');var c=b.data("targetSelector");b=b.data("toggler");var g=a(this).text();if("undefined"!==c){var d=a(c);c=
...
c+' [data-type="emojiPopper"]');b.removeAttr("aria-describedby").attr("aria-label",f.title);a(c).data({targetSelector:b.data("target"),toggler:b})});l.on("show.bs.popover",function(){a(this).removeAttr("aria-describedby").attr("aria-label",f.title)});if(f.dismiss)a("body").on("click",function(b){l.each(function(){a(this).is(b.target)||0!==a(this).has(b.target).length||0!==a(".popover").has(b.target).length||a(this).popover("hide")})});return this}var f=a.extend({},{title:"Pick an emoji...",storageType:"localStorage",
